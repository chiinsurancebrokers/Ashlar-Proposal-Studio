...
def _apply_deterministic_facts(result: dict, provider_label: str, quotation_text: str, focused_context: str) -> dict:
    ...
    # IMPORTANT: match the actual benefit row, not a neighbouring row whose label merely
    # contains the words "excluding Advanced Medical Imaging". The latter caused Cigna
    # Silver to inherit the hospital-accommodation value "Private room".
    advanced_imaging = next(
        (r for r in rows if re.match(r"^advanced medical imaging\b", r[1].strip(), re.I)),
        None,
    )
    ct_pet_mri = _find_row(rows, "ct", "pet", "mri")
    outpatient_diagnostic = next((r for r in rows if r[1].strip().casefold() == "diagnostic"), None)

    # Exact deterministic table evidence outranks LLM prose. Override a model-generated
    # value when the selected-plan row exists; this also prevents stale table spill-over.
    if advanced_imaging:

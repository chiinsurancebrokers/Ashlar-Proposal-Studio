...
def validate_client_report(report: dict, results: list[dict]) -> dict:
    ...
    if len(expected) > 1:
        ...
        if not str(ass.get("recommended_provider") or "").strip() or not str(ass.get("recommended_plan") or "").strip():
            raise ClientReportValidationError(
                "Ashlar advisory report must contain one clear recommended option when the comparison facts passed the quality gate."
            )

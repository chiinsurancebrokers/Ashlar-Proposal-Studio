...
- Ashlar now acts explicitly as a broker-adviser: comparative reports must contain one clear **Recommended option** after the factual quality gate passes.
- The report also distinguishes a **Strong alternative**, a **Broader extras option** and a **Budget option** when the verified case facts support those routes.
- Deterministic premium ordering is included in the decision payload, preventing simple superlative mistakes such as calling a plan the highest premium when another quote is more expensive.
- Waiting-period statements are benefit-bound: a dental waiting period cannot leak into surgery, maternity or another unrelated benefit.
- The assessment slide/PDF now uses direct client-facing labels rather than a neutral “no single plan wins” conclusion.
- A comparative report with no primary recommendation is rejected before it replaces the previous validated report.

## v0.5.0 — FastAPI reliability architecture

- Streamlit is now the presentation layer; long-running client-report and HAL jobs can run through a separate FastAPI service + worker.
- Supabase `analysis_jobs` is a persistent queue, so report generation survives normal Streamlit reruns and the UI no longer has to wait inside the model call.
- Client reports are validated with Pydantic before replacing a previous report. Empty or partial JSON is rejected.
- Each comparative report must contain a narrative for every analyzed plan plus a populated Ashlar Assessment and next steps.
- The report writer receives a compact evidence digest (max 14 supporting items per plan) instead of hundreds of extraction rows.
- Claude report generation has an explicit timeout and lower output ceiling.
- HAL can use the same FastAPI/worker job path.
- Existing invalid/partial saved reports are hidden and can be regenerated without touching the factual plan analysis.

See `FASTAPI_V050.md` for the Railway three-service setup and run `supabase/jobs_schema.sql` once.

## v0.4.11 — report cleanup and FMU consistency

- PDF cover no longer repeats the Executive Summary from page 2.
- Full Medical Underwriting comparisons use one client-facing rule for pre-existing conditions: subject to underwriting approval and excluded unless specifically accepted by the insurer in writing.
- CPME, MHD and moratorium retain their own underwriting-specific wording.
- The report no longer infers health or pre-existing-condition likelihood from age alone.
- More tolerant narrative matching prevents blank plan-review pages when provider labels differ slightly (for example `CIGNA SILVER` vs `CIGNA`).
...
## v0.4.8 — Applicant-aware client reporting

This release tightens the client-facing narrative layer:

- adds Applicant sex and age fields used for relevance filtering;
- removes maternity/pregnancy/newborn discussion for male applicants;
- separates Diagnostics / Advanced Imaging from Preventive / Wellness;
- locks IMG CT/PET/MRI evidence separately from the wellness allowance;
- simplifies zero member cost-sharing to the quoted deductible/excess in client outputs;
- adds a per-option underwriting-basis broker override (including Full Medical Underwriting);
- limits Important Considerations to applicant-relevant waiting periods, limits, exclusions and usage conditions;
- removes broker-internal housekeeping from client-facing considerations/next steps;
- carries applicant demographics into HAL and Saved Cases;
- adds Supabase Case Archive columns `client_sex` and `client_age` (re-run `supabase/cases_schema.sql` once to apply the safe `ADD COLUMN IF NOT EXISTS` migration).

The model is also explicitly prohibited from inventing visit counts, healthcare-cost scenarios or risk probabilities not present in the supplied evidence.

## v0.4.9 — Underwriting-aware Next Steps

Client-report Next Steps are now deterministic when the underwriting method is known.

For **Full Medical Underwriting (FMU)** the report always follows the real client journey:

1. complete the insurance application and full medical history declaration;
2. submit the case for insurer medical underwriting (with additional evidence if requested);
3. review the insurer's final applicant-specific underwriting terms, exclusions/special conditions, confirmed premium and effective date;
4. accept the final terms and activate the policy.

Equivalent client workflows are also defined for Moratorium, CPME and MHD. The LLM may write the advisory narrative, but it can no longer omit or replace the correct underwriting workflow when the method is known.

## v0.4.10
See `DIAGNOSTICS_WELLNESS_FIX.md` for the Cigna diagnostics and Now Health wellness correction.


## v0.5.2 — Reliable long comparative reports

See `REPORT_COMPLETION_V052.md`. Large 4-provider reports now use adaptive output budgets, detect token truncation explicitly, retry once with a larger budget, and refuse nested partial JSON objects. `CLIENT_ANALYSIS_MODEL=claude-sonnet-5` is recommended for final client-facing reports; HAL can remain on Haiku.


## v0.5.4 modular report generation
Client reports now use deterministic plan pages and a compact LLM decision-synthesis step. See `MODULAR_REPORT_V054.md`.

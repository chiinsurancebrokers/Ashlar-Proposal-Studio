...
def build_pptx_bytes(*, client_analysis: dict, results: list[dict], language: str = "English") -> bytes:
    ...
    for start in range(0, len(results), 4):
        ...
        for idx, result in enumerate(subset):
            ...
            _text(s, _compact(a.get("deductible_or_excess"), 48), x+.22, 6.02, card_w-.44, .34, 10, False)
        _footer(s)

    # Each plan
    narrative_by_key = {(p.get("provider"), p.get("plan_name")): p for p in client_analysis.get("plans", [])}
    for result in results:
        a = result.get("analysis") or {}
        n = narrative_by_key.get((a.get("provider") or result.get("provider"), a.get("plan_name") or result.get("target_plan"))) or {}
        ...
        facts = [
            ...
            ("Excess", a.get("deductible_or_excess")),

...
LLM JSON handling now uses a tolerant first-object decoder instead of calling
`json.loads()` on the entire model response. This prevents Railway case analysis
from crashing when a model returns valid JSON followed by explanatory text or a
second accidental JSON object (`JSONDecodeError: Extra data`). Malformed output
that cannot be recovered is shown as a broker-facing diagnostic instead of
terminating the Streamlit session.

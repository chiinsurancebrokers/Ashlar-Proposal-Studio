...
from core.api_client import AshlarAPIError, api_configured, enqueue_chat, enqueue_report, fetch_job
...
def _reset_case_session() -> None:
    for key in [
        ...
        "client_report_job_id", "client_report_job_error", "hal_job_id", "hal_job_error", "hal_pending_question",
    ]:
...
def _background_jobs_enabled() -> bool:
    return api_configured() and archive_configured()


def _sync_case_outputs_from_archive(*, report: bool = False, chat: bool = False) -> None:
    case_id = st.session_state.get("current_case_id")
    if not case_id:
        return
...
def _render_job_status(*, job_key: str, error_key: str, kind: str) -> None:
    ...
    def render_once() -> None:
        ...
        except AshlarAPIError as exc:
            st.warning(f"Background service status unavailable: {exc}")
            return

        status = job.get("status") or "pending"
        stage = job.get("progress_stage") or "Working…"
        if status in {"pending", "running"}:
            st.info(f"⏳ {stage}")
            return
        if status == "completed":
            try:
                _sync_case_outputs_from_archive(report=(kind == "report"), chat=(kind == "chat"))
            except CaseArchiveError as exc:
                ...
                if kind == "chat":
                    st.session_state.pop("hal_pending_question", None)
                st.success("Client report ready." if kind == "report" else "HAL response ready.")
                st.rerun()
            return

        # Failed jobs never replace the previously validated report/chat.
        st.session_state[error_key] = job.get("error_message") or "Background job failed."
        st.session_state.pop(job_key, None)
        if kind == "chat":
            st.session_state.pop("hal_pending_question", None)
        st.error(st.session_state[error_key])

    # Streamlit fragments rerun independently, so the rest of the Case Studio stays
    # interactive while FastAPI/worker processes the long-running job.
    if hasattr(st, "fragment"):
        ...
        render_once()
        if st.button("Refresh background job status", key=f"refresh_{job_key}"):
            st.rerun()


def _run_hal_question(question: str, results: list[dict]) -> None:
    """Run one grounded HAL turn, asynchronously when FastAPI is configured."""
    question = (question or "").strip()
    if not question:
        return

    if _background_jobs_enabled():
        case_id = _persist_current_case(
            silent=True,
            status="report_ready" if st.session_state.get("client_analysis") else "analyzed",
        )
        if not case_id:
            st.session_state["hal_job_error"] = "Save the analyzed case before sending it to HAL."
            return
        try:
            queued = enqueue_chat(case_id, question)
        except AshlarAPIError as exc:
            st.session_state["hal_job_error"] = str(exc)
            return
        st.session_state["hal_job_id"] = queued.get("job_id")
        st.session_state["hal_pending_question"] = question
        st.session_state["hal_job_error"] = ""
        return

    # Safe synchronous fallback for local development or before the API service is enabled.
...
def render_case_chat(results: list[dict]) -> None:
    ai_ready = bool(os.getenv("ANTHROPIC_API_KEY")) or _background_jobs_enabled()
    ...
    _render_job_status(job_key="hal_job_id", error_key="hal_job_error", kind="chat")
    pending_question = st.session_state.get("hal_pending_question")
    if pending_question and st.session_state.get("hal_job_id"):
        st.caption(f"Queued for HAL: {pending_question}")
    ...
    q1, q2 = st.columns(2)
    with q1:
        if st.button("Key differences", key="quick_diff", use_container_width=True, disabled=(not ai_ready) or bool(st.session_state.get("hal_job_id"))):
            _run_hal_question(
            ...
            st.rerun()
    with q2:
        if st.button("Risk flags", key="quick_risk", use_container_width=True, disabled=(not ai_ready) or bool(st.session_state.get("hal_job_id"))):
            _run_hal_question(
    ...
    with st.form("case_chat_form", clear_on_submit=True, enter_to_submit=False):
        question = st.text_area(
            ...
            disabled=(not ai_ready) or bool(st.session_state.get("hal_job_id")),
        )
...
def render_client_deliverable(results: list[dict]) -> None:
    ...
    report_job_active = bool(st.session_state.get("client_report_job_id"))
    ...
    with c1:
        generate = st.button(
            ...
            type="primary",
            use_container_width=True,
            disabled=(not reviewed) or (not quality["can_generate"]) or report_job_active,
        )
    with c2:
        mode = "FastAPI background worker" if _background_jobs_enabled() else "local synchronous fallback"
        st.caption(f"Generation mode: {mode}. A previous validated report remains visible until a new one passes validation.")

    if generate:
        if _background_jobs_enabled():
            case_id = _persist_current_case(silent=True, status="report_queued")
            if not case_id:
                st.session_state["client_report_job_error"] = "The case could not be saved before report generation."
            else:
                try:
                    queued = enqueue_report(case_id)
                except AshlarAPIError as exc:
                    st.session_state["client_report_job_error"] = str(exc)
                else:
                    st.session_state["client_report_job_id"] = queued.get("job_id")
                    st.session_state["client_report_job_error"] = ""
            st.rerun()
        else:
            # Local fallback still validates the complete report before replacing the previous one.
            try:
                with st.spinner("Building and validating the client-facing comparison…"):
                    ...
                    validate_client_report(generated, results)
            except ClientReportValidationError as exc:
                st.error(f"The new report was rejected and the previous report was preserved. {exc}")
            else:
                st.session_state["client_analysis"] = generated
                _persist_current_case(silent=True, status="report_ready")
                st.rerun()

    _render_job_status(job_key="client_report_job_id", error_key="client_report_job_error", kind="report")

    report = st.session_state.get("client_analysis")
    if not report:
        return

    # Never present a partial/empty stored narrative as a successful client report.
    try:
        validate_client_report(report, results)
    except ClientReportValidationError as exc:
        st.error(
            "The stored client narrative is incomplete and has been hidden. "
            f"Generate it again with v0.5; the factual plan analysis remains intact. Technical detail: {exc}"
        )
        return
